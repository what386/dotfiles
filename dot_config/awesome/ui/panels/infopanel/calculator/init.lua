...
local format_screen = function(value)
	...
	elseif
		calcu_screen:get_text() == "inf"
		or calcu_screen:get_text() == "undefined"
		or calcu_screen:get_text() == "SYNTAX ERROR"
	then
...
local build_button_widget = function(text, rcp, rad)
	...
	build_button:buttons(gears.table.join(awful.button({}, 1, nil, function()
		if value == "C" then
			clear_screen()
		elseif value == "=" then
...
local toggle_btn_keygrab = function()
	if keygrab_running then
		kb_imagebox:set_image(icons.system.kb_off)
		awesome.emit_signal("widget::calc_stop_keygrab")
		keygrab_running = false
	else
		kb_imagebox:set_image(icons.system.kb_on)
...
kb_button:buttons(gears.table.join(awful.button({}, 1, nil, function()
	toggle_btn_keygrab()
end)))
