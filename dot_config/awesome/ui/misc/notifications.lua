...
local function dnd_enabled()
	return _G.dont_disturb_state == true or _G.dont_disturb == true
end

local function play_notification_sound(n)
	if dnd_enabled() then
		return
	end

	local critical = n.urgency == "critical"
	local event = critical and "notification_critical" or "notification"

	if sounds.play(event) then
		return
	end

	local canberra_event = critical and "dialog-warning" or "message"
	awful.spawn.with_shell("canberra-gtk-play -i " .. canberra_event)
end
