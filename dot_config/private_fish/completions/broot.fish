...
complete -c broot -l max-depth -d 'Only show trees up to a certain depth' -r
...
complete -c broot -l color -d 'Whether to have styles and colors' -r -f -a "auto\t''
yes\t''
no\t''"
complete -c broot -l height -d 'Height (if you don\'t want to fill the screen or for file export)' -r
complete -c broot -l set-install-state -d 'Manually set installation state' -r -f -a "undefined\t''
refused\t''
installed\t''"
...
complete -c broot -l listen-auto -d 'create a random socket to listen to for commands'
