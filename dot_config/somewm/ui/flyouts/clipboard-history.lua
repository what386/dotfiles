...
local function item_matches(item, q)
	...
	return item.text:lower():find(q:lower(), 1, true) ~= nil
end
...
local function rebuild_list()
	...
	for _, item in ipairs(clipboard.get_items()) do
		if item_matches(item, query) then
			table.insert(visible_items, item)
		end
		if #visible_items >= MAX_ROWS then break end
	end
...
local function close_popup()
	if grabber then grabber:stop(); grabber = nil end
	popup.visible = false
end
...
local function show_popup()
	...
	grabber = awful.keygrabber({
		...
		keypressed_callback = function(_, mods, key)
			...
			elseif key == "Up"   or key == "Left"        then move_selection(-1)
			elseif key == "Down" or key == "Right"       then move_selection(1)
			elseif key == "Tab"                          then move_selection(has_mod(mods, "Shift") and -1 or 1)
			elseif key == "ISO_Left_Tab" or key == "BackTab" then move_selection(-1)
			elseif key == "Return"                       then choose_selected(not has_mod(mods, "Control"))
			end
		end,
	})
end
