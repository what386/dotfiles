...
local function build_section(label, count, packages)
	...
	for i, pkg in ipairs(packages) do
		if i > 10 then
			text = text .. string.format("• ... and %d more\n", count - 10)
			break
		end
		text = text .. "• " .. pkg .. "\n"
	end
...
local function update_widget_state(pacman_count, pacman_packages, aur_count, aur_packages, upstream_count, upstream_packages)
	...
	if total_updates > 0 then
		...
		tooltip_text = tooltip_text .. build_section("Pacman", pacman_count, pacman_packages)
		tooltip_text = tooltip_text .. build_section("AUR", aur_count, aur_packages)
		tooltip_text = tooltip_text .. build_section("Upstream", upstream_count, upstream_packages)
...
local function check_updates()
	...
	local function on_check_complete()
		...
		if checks_completed == 3 then
			update_widget_state(
				pacman_count, pacman_packages,
				aur_count, aur_packages,
				upstream_count, upstream_packages
			)
		end
	end
