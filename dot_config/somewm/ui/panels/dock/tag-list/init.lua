    ...
    if buttons then
        ...
        for _, b in ipairs(buttons) do
            btns[#btns + 1] = awful.button({
                ...
                button    = b.button,
                on_press  = function() if b.on_press then b.on_press(object) end end,
                on_release = function() if b.on_release then b.on_release(object) end end,
            })
        end
        return btns
    end
		...
		awful.widget.taglist.filter.all,
{
	...
	local underline_anim = rubato.timed({
		...
		clamp_position = true,  -- prevents overshooting the target
