...
function AuthManager:new()
	...
	self.capslock_on = false   -- Track caps lock state internally
	self:_init_pam()
	return self
end
...
function AuthManager:_start_password_grabber(password_widget, capslock_widget)
	...
	-- Wayland-compatible caps lock detection.
	--
	-- `xset` is X11-only and unavailable under a native Wayland compositor.
	-- Instead we track the Caps_Lock toggle ourselves: each Caps_Lock keypress
	-- flips `self.capslock_on`, which is initialised to false when the
	-- AuthManager is constructed.  This is reliable as long as the keygrabber
	-- is active (i.e. while the lockscreen is shown), which is the only time
	-- we need the indicator.
	--
	-- If you are running AwesomeWM under XWayland and need the *system* state
	-- at the moment the lockscreen first appears, replace the body of
	-- `check_capslock` with an `awful.spawn.easy_async` call to
	-- `numlockx status` or read /sys/class/leds/input*::capslock/brightness
	-- (the LED path varies by keyboard; enumerate with
	-- `ls /sys/class/leds/*capslock*/brightness 2>/dev/null`).
	local function check_capslock()
		capslock_widget.opacity = self.capslock_on and 1.0 or 0.0
		capslock_widget:emit_signal("widget::redraw_needed")
	end

	-- Optionally seed the initial state from the kernel LED (best-effort,
	-- silently ignored if the sysfs path does not exist on this machine).
	awful.spawn.easy_async_with_shell(
		"cat /sys/class/leds/*capslock*/brightness 2>/dev/null | head -1",
		function(stdout)
			local val = stdout and stdout:match("^%s*(%d+)") or nil
			if val then
				self.capslock_on = tonumber(val) ~= 0
			end
			check_capslock()
		end
	)
	...
	self.password_grabber = awful.keygrabber({
		...
		keyreleased_callback = function(_, _, key)
			if key == "Caps_Lock" then
				-- Toggle our internal tracker and refresh the indicator.
	...
	check_capslock() -- Initial check (uses sysfs seed above when available)
end
