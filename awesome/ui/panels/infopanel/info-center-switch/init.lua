...
local wrap_notif = wibox.widget({
	...
	shape = function(cr, width, height)
		gears.shape.partially_rounded_rect(cr, width, height, false, true, true, false, beautiful.groups_radius)
	end,
...
local today_button = clickable_container(wibox.container.margin(today_text, dpi(0), dpi(0), dpi(7), dpi(7)))
...
local wrap_today = wibox.widget({
	...
	border_color = beautiful.groups_title_bg,
	shape = function(cr, width, height)
		gears.shape.partially_rounded_rect(cr, width, height, true, false, false, true, beautiful.groups_radius)
	end,
	widget = wibox.container.background,
})
...
function switch_rdb_pane(infopanel_mode)
	local focused = awful.screen.focused()

	if infopanel_mode == "notif_mode" then
		-- Update button color
		wrap_notif.bg = active_button
		wrap_today.bg = inactive_button

		-- Change panel content of right-panel.lua
		focused.infopanel:switch_pane(infopanel_mode)
	elseif infopanel_mode == "today_mode" then
		-- Update button color
...
notif_button:buttons(gears.table.join(awful.button({}, 1, nil, function()
	switch_rdb_pane("notif_mode")
end)))

today_button:buttons(gears.table.join(awful.button({}, 1, nil, function()
