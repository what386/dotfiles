name: Release
on:
  push:
    tags:
      - "v*"
permissions:
  contents: write
jobs:
  build:
    name: Build ($%{{ matrix.os }}%)
    runs-on: $%{{ matrix.os }}%
    strategy:
      matrix:
        include:
          - os: ubuntu-latest
            target: x86_64-unknown-linux-gnu
            binary_ext: ""
          - os: ubuntu-latest
            target: aarch64-unknown-linux-gnu
            binary_ext: ""
          - os: macos-latest
            target: x86_64-apple-darwin
            binary_ext: ""
          - os: macos-latest
            target: aarch64-apple-darwin
            binary_ext: ""
          - os: windows-latest
            target: x86_64-pc-windows-msvc
            binary_ext: ".exe"
          - os: windows-latest
            target: aarch64-pc-windows-msvc
            binary_ext: ".exe"
    steps:
      - uses: actions/checkout@v6
      - uses: dtolnay/rust-toolchain@stable
        with:
          targets: $%{{ matrix.target }}%
      - name: Install cross-compilation tools
        if: matrix.target == 'aarch64-unknown-linux-gnu'
        run: |
          sudo apt-get update
          sudo apt-get install -y gcc-aarch64-linux-gnu
      - name: Build
        run: cargo build --release --target $%{{ matrix.target }}%
        env:
          CARGO_TARGET_AARCH64_UNKNOWN_LINUX_GNU_LINKER: aarch64-linux-gnu-gcc
      - name: Archive binary
        run: |
          mkdir dist
          cp target/$%{{ matrix.target }}%/release/{{forge.project.name}}$%{{ matrix.binary_ext }}% dist/{{forge.project.name}}-$%{{ matrix.target }}%$%{{ matrix.binary_ext }}%
        shell: bash
      - name: Upload artifact
        uses: actions/upload-artifact@v7
        with:
          name: {{forge.project.name}}-$%{{ matrix.target }}%
          path: dist/{{forge.project.name}}-$%{{ matrix.target }}%$%{{ matrix.binary_ext }}%
  release:
    name: Create GitHub Release
    needs: build
    runs-on: ubuntu-latest
    steps:
      - uses: actions/checkout@v6
      - name: Download all artifacts
        uses: actions/download-artifact@v8
        with:
          path: dist
      - name: Add completion assets
        run: |
          cp completions/completions.bash dist/completions.bash
          cp completions/completions.fish dist/completions.fish
          cp completions/completions.zsh dist/completions.zsh
          cp completions/completions.ps1 dist/completions.ps1
          cp completions/completions.elvish dist/completions.elvish
      - name: Generate checksums
        run: |
          cd dist
          sha256sum **/* > SHA256SUMS.txt
      - name: Read changelog section for tag
        id: release_notes
        run: |
          RELEASE_NOTES="$(./scripts/changelog_for_tag.bash "$%{{ github.ref_name }}%")"
          {
            echo "body<<__RELEASE_NOTES__"
            printf '%s\n' "$RELEASE_NOTES"
            echo "__RELEASE_NOTES__"
          } >> "$GITHUB_OUTPUT"
      - name: Create release
        uses: softprops/action-gh-release@v3
        with:
          tag_name: $%{{ github.ref_name }}%
          body: $%{{ steps.release_notes.outputs.body }}%
          files: dist/**/*
